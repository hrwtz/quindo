...
function quindo_scripts_and_styles() {
    ...
    if (!is_admin()) {
        ...
        if (is_page_template('page-patterns.php')) :
            wp_enqueue_style( 'patterns', get_template_directory_uri() . '/_/css/patterns'.$quindo_file_suffix.'.css?'.filemtime(get_template_directory()."/_/css/patterns".$quindo_file_suffix.".css"), false );
        endif;
    }
}
add_action( 'wp_enqueue_scripts', 'quindo_scripts_and_styles', 16 );

// Set Inline scripts in header
function quindo_head(){

}
add_action( 'wp_head', 'quindo_head' );

function quindo_footer(){
    
}
add_action( 'wp_head', 'quindo_footer' );
